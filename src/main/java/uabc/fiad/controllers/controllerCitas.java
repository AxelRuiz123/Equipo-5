 ...
 * @author kevin
 */
@WebServlet(name = "controllerCitas", urlPatterns = {"/controllerCitas"})
public class controllerCitas extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            /*
           Enumeration<String> enc = request.getHeaderNames();
           while(enc.hasMoreElements()){
               String name = enc.nextElement();
           ...
           */
            ...
            String nombre = archivo.substring(archivo.lastIndexOf("/")+1, archivo.length() - 4);
            System.out.println("nombre archivo = " + nombre);

            if(nombre.equals("citas")){
                ArrayList<Paciente> lista = new ArrayList();           
              lista = Paciente.consultaPacientes();
           
               request.setAttribute("jb", lista);
               RequestDispatcher rd = request.getRequestDispatcher("citas.jsp");
               rd.forward(request, response);
            }else{
                out.println("Enviar a otra vista");
            }
        } finally {
            out.close();
        }
